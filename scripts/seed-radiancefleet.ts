/**
 * Seed script for radiancefleet-ops project.
 *
 * Creates:
 *   - Project: radiancefleet (slug: radiancefleet)
 *   - Departments: ops, core
 *   - Agent keys: radiancefleet-ops-agent, radiancefleet-core-agent
 *   - Permissions: cross-department collaboration with can_comment
 *
 * Usage:
 *   SUPABASE_SERVICE_ROLE_KEY=<key> deno run --allow-net --allow-env scripts/seed-radiancefleet.ts
 *
 * Environment:
...
async function main() {
  console.log('Seeding radiancefleet-ops...\n')
  ...
  const project = await getOrCreateProject(workspaceId, 'RadianceFleet', 'radiancefleet')
  ...
  if (opsAgent.fullKey) {
    console.log(`\n  radiancefleet-ops-agent:`)
    console.log(`    ${opsAgent.fullKey}`)
  } else {
    console.log(`\n  radiancefleet-ops-agent: (already existed — secret not available)`)
  }
  if (coreAgent.fullKey) {
    console.log(`\n  radiancefleet-core-agent:`)
    console.log(`    ${coreAgent.fullKey}`)
  } else {
    console.log(`\n  radiancefleet-core-agent: (already existed — secret not available)`)
  }
  console.log('\n' + '='.repeat(60))
  console.log('Done!')
}
