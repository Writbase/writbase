...
interface SidebarProjectSelectorProps {
  activeProjects: Array<{ id: string; name: string; is_archived: boolean }>;
    ...
    <>
      ...
      <div>
        <div className="flex items-center justify-between">
          ...
          <button onClick={onAddProject} className="text-xs text-blue-400 hover:text-blue-300">
            + Add
          </button>
        </div>
          ...
          {activeProjects.map((p) => (
            ...
            >
              ...
              <span className="flex shrink-0 gap-1 opacity-0 transition-opacity group-hover:opacity-100">
                <button
                  onClick={() => {
                    onEditProject({ id: p.id, name: p.name });
                  }}
